...
CLIENT_TYPES = {
    "angry": {
        "system_prompt": "Ты покупатель на Авито. РЕАЛЬНЫЙ ЧЕЛОВЕК. Недоверчивый, прямой, иногда резкий. Торгуешься жёстко. Задавай вопросы о товаре. Реагируй на ответы. НЕ ПОВТОРЯЙСЯ. Отвечай 1-3 предложения как в чате.",
        "discount_range": (0.6, 0.8), "patience": 4, "remind_time": (120, 300)
    },
    "kind": {
        "system_prompt": "Ты покупатель на Авито. РЕАЛЬНЫЙ ЧЕЛОВЕК. Вежливый. Просишь скидку 5-15%. Задавай вопросы. Хвали товар. Реагируй на ответы. НЕ ПОВТОРЯЙСЯ. Отвечай 1-3 предложения как в чате.",
        "discount_range": (0.85, 0.95), "patience": 6, "remind_time": (180, 420)
    },
    "sly": {
        "system_prompt": "Ты покупатель-перекупщик. РЕАЛЬНЫЙ ЧЕЛОВЕК. Знаешь рынок. Хитрый. Аргументируешь цену. Задавай вопросы. Реагируй на ответы. НЕ ПОВТОРЯЙСЯ. Отвечай 1-3 предложения как в чате.",
        "discount_range": (0.7, 0.85), "patience": 5, "remind_time": (150, 360)
    }
}

# ==================== ПОДРАБОТКИ (ДЕТАЛЬНЫЕ) ====================
JOBS = [
    {
        "id": "flyers",
        "name": "📦 Расклейка объявлений",
        "description": "Расклеиваешь объявления по району. Платят немного, зато быстро.",
        "duration": 60,
        "reward": 200,
        "emoji": "📦",
        "steps": ["📦 Взял пачку объявлений...", "🏃 Бежишь по району...", "📌 Клеишь на доски...", "✅ Готово! Собрал все доски."]
    },
    {
        "id": "delivery",
        "name": "🚗 Доставка заказов",
        "description": "Развозишь заказы на велосипеде. Средний заработок.",
        "duration": 120,
        "reward": 500,
        "emoji": "🚗",
        "steps": ["🚗 Принял заказ в приложении...", "📦 Забираешь посылку...", "🛵 Едешь к клиенту...", "📱 Отмечаешь доставку...", "✅ Заказ доставлен!"]
    },
    {
        "id": "freelance",
        "name": "💻 Фриланс (дизайн)",
        "description": "Делаешь логотип для клиента. Платят хорошо, но дольше.",
        "duration": 300,
        "reward": 1200,
        "emoji": "💻",
        "steps": ["💻 Открыл редактор...", "🎨 Подбираешь цвета...", "✏️ Рисуешь макет...", "📤 Отправляешь клиенту...", "🔄 Правки от клиента...", "✅ Клиент доволен!"]
    },
    {
        "id": "shop",
        "name": "🏪 Работа в магазине",
        "description": "Подменяешь продавца в стоке. Можно и товар присмотреть.",
        "duration": 180,
        "reward": 700,
        "emoji": "🏪",
        "steps": ["🏪 Пришёл на смену...", "📦 Разбираешь товар...", "👥 Консультируешь покупателей...", "💰 Пробиваешь на кассе...", "✅ Смена окончена!"]
    },
    {
        "id": "photo",
        "name": "📸 Съёмка товаров",
        "description": "Фоткаешь вещи для других продавцов. Быстрые деньги.",
        "duration": 90,
        "reward": 350,
        "emoji": "📸",
        "steps": ["📸 Настроил свет...", "👕 Раскладываешь вещи...", "📷 Делаешь кадры...", "💻 Обрабатываешь фото...", "✅ Отправил заказчику!"]
    },
]
...
# Подработки
side_jobs = {}  # {user_id: {"job_type": str, "start_time": float, "step": int, "done": bool}}
job_notifications = {}  # {user_id: asyncio.Task}
...
def complete_lesson(user_id, lesson_id):
    ...
    if lesson_id not in l["completed"]:
        l["completed"].append(lesson_id); u["lessons_completed"] = len(l["completed"]); l["current"] = lesson_id + 1
...
def get_player(user_id):
    if user_id not in players:
        r = get_rep(user_id); rm = rep_mult(r["score"])
        players[user_id] = {"balance": 5000, "reputation": max(0, r["score"]), "inventory": [], "day": 1, "total_earned": 0, "total_spent": 0, "items_sold": r["total_sales"], "scam_times": r["scam_survived"], "market_demand": {cat: 1.0 for cat in CATEGORIES}, "current_event": None, "stat_earned_today": 0, "stat_sold_today": 0, "rep_mult": rm}
...
async def spawn_buyers(user_id):
    ...
    for i, bt in enumerate(types):
        await asyncio.sleep(random.randint(5, 20)); bid = i + 1
...
async def complete_sale(user_id, buyer_id, message=None):
    ...
    if user_id in published_items and published_items[user_id] and published_items[user_id]["item"]["name"] == item_name:
        sold = published_items[user_id]["item"]; published_items[user_id] = None
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if len(args) > 1 and args[1].startswith("ref_"):
        ref_code = args[1][4:]; referrer_id = None
    ...
    if p and p.get("day", 0) > 0:
        r = get_rep(user_id)
        await send_msg(user_id, f"👋 <b>С ВОЗВРАЩЕНИЕМ!</b>\n📅 День {p['day']} | 💰 {p['balance']}₽\n⭐ {rep_level(r['score'])}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🎮 ПРОДОЛЖИТЬ", callback_data="continue_game")], [InlineKeyboardButton(text="📚 ОБУЧЕНИЕ", callback_data="action_learn")]]))
    else:
        l = get_learning(user_id)
        await send_msg(user_id, f"🎮 <b>RESELL TYCOON</b>\n\nНаучись зарабатывать на перепродаже!\n📚 Уроков: {len(l['completed'])}/{len(LESSONS)}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📚 ОБУЧЕНИЕ", callback_data="action_learn")], [InlineKeyboardButton(text="🚀 НАЧАТЬ ИГРУ", callback_data="start_new_game")], [InlineKeyboardButton(text="🔗 РЕФЕРАЛЫ", callback_data="ref_info")]]))

@dp.message(Command('play'))
async def play_cmd(message: types.Message, state: FSMContext):
    user_id = message.from_user.id; await del_user_msgs(user_id)
    r = get_rep(user_id)
    players[user_id] = {"balance": 5000, "reputation": max(0, r["score"]), "inventory": [], "day": 1, "total_earned": 0, "total_spent": 0, "items_sold": r["total_sales"], "scam_times": r["scam_survived"], "market_demand": {cat: 1.0 for cat in CATEGORIES}, "current_event": None, "stat_earned_today": 0, "stat_sold_today": 0, "rep_mult": rep_mult(r["score"])}
    p = players[user_id]
...
async def process_chat(user_id, chat_key, text, message):
    ...
    client_type = chat["client_type"]; client = CLIENT_TYPES[client_type]
...
async def rep_menu_callback(callback: CallbackQuery):
    ...
    new_a = check_ach(user_id, {"items_sold": p["items_sold"], "total_earned": p["total_earned"]} if p else None)
    u = get_rep(user_id); lvl = rep_level(u["score"])
...
async def learn_btn(callback: CallbackQuery):
    user_id = callback.from_user.id; l = get_learning(user_id)
    kb = []
...
async def buy_item(callback: CallbackQuery, state: FSMContext):
    ...
    check_ach(user_id, {"items_sold": p["items_sold"], "total_earned": p["total_earned"]})
    save_json(REPUTATION_FILE, rep_data)
    await edit_msg(callback.message, f"✅ <b>КУПЛЕНО!</b>\n📦 {item['cat']} {item['name']}\n💰 Закуп: {price}₽ | 📊 ~{mp}₽\n💼 Баланс: {p['balance']}₽\n\n👇 Зайди в 📦 ИНВЕНТАРЬ и опубликуй!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="📦 В ИНВЕНТАРЬ", callback_data="action_inventory")], [InlineKeyboardButton(text="🏠 МЕНЮ", callback_data="action_back")]]))
...
async def publish_item(callback: CallbackQuery):
    ...
    await edit_msg(callback.message, f"📢 <b>ОПУБЛИКОВАНО!</b>\n📦 {item['name']}\n💰 {item['market_price']}₽\n\n⏳ Жди 1-3 минуты — придут покупатели!", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="💬 ЧАТЫ", callback_data="action_chats")], [InlineKeyboardButton(text="💼 ЗАРАБОТОК", callback_data="action_job")], [InlineKeyboardButton(text="🏠 МЕНЮ", callback_data="action_back")]]))
